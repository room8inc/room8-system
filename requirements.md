...
- 個人契約がベース。契約後、メイン会員がダッシュボードからメンバーを追加できる
- メンバーは50% OFFで利用可能
- プラン管理・メンバー管理はすべてメイン会員が行う
- メンバーのアカウントはメイン会員が直接作成（パスワード設定、メール招待不要）

#### メンバー追加

- メイン会員がメンバーの姓・名・メールアドレス・パスワード・プランを指定して追加
- メンバーのプランはメイン会員のプラン以下の価格に制限
- アカウント作成後、ログイン情報（メール・パスワード）がメイン会員に表示される
- メイン会員がメンバーに直接ログイン情報を伝える運用
...
- 会議室予約
- プロフィール確認
- ※プラン変更・メンバー管理はメイン会員のみ

#### Stripe連携

- メイン会員のサブスクリプション1本にメンバー分のアイテムを追加
- 50% OFFクーポン（group_50off）が自動適用
- メンバー削除時はサブスクリプションアイテムのquantityを減算/削除

#### API

- `POST /api/members/invite` — メンバー追加（アカウント作成 + プラン作成 + Stripeアイテム追加）
- `GET /api/members/list` — メンバー一覧取得
- `DELETE /api/members/remove` — メンバー削除（プラン終了 + Stripeアイテム削除）
- `PUT /api/members/change-plan` — メンバーのプラン変更

#### UI

- `/members` — メンバー管理ページ（メンバー一覧・追加フォーム・削除・月額合計表示）
- ダッシュボードに「メンバー管理」ボタン表示（アクティブプラン保有時）
