...
- **座席管理システム**
  ...
  - 会議室座席: 8席（会議室予約がない時のみ利用可能）
  - 最大27席（会議室予約がない場合）
  - 会場にチェックインしている必要がある（座席チェックインの前提条件）
  - 出口QRコードでチェックアウト時に座席も自動解除
...
- **座席構成**:
  ...
  - 最大27席（会議室予約がない場合）
- **座席チェックイン方法**:
  - ダッシュボードの座席表をタップして座席にチェックイン
  - 会場にチェックインしている必要がある（前提条件）
- **座席チェックアウト方法**:
  - 自分の座席をタップしてチェックアウト
  - または出口QRコードで会場チェックアウト時に自動解除
- **会議室座席の制御**:
  - 会議室予約がある場合 → 会議室座席は使用不可
  - 会議室予約がない場合 → 会議室座席も利用可能
