...
個人契約をベースに、メイン会員がメンバーを追加できる仕組み。旧グループプラン（group_plans/group_slots/group_members テーブル）は廃止済み。

### 仕組み

- メイン会員がダッシュボードの「メンバー管理」からメンバーを追加
- メンバーのアカウントは `admin.createUser` で即時作成（メール招待・承認プロセスなし）
- パスワードはメイン会員が指定（空欄の場合は自動生成）
- 作成後、ログイン情報（メール・パスワード）が画面に表示される → メイン会員がメンバーに口頭等で伝える
- メンバーは自分の `user_plans` レコードを持つため、既存のチェックインフローがそのまま動く

### DB設計

- `user_plans.invited_by` — 招待元ユーザーID（NULLなら通常契約）
- `user_plans.discount_code` — 適用された割引（'group_50off'）
- RLS: ホストは `invited_by = auth.uid()` でメンバーのプランをSELECT可能
- INSERT/UPDATEはservice client経由（ホストが別ユーザーのレコードを操作するため）

### Stripe連携

- メイン会員のサブスクリプションにメンバー分のアイテムを追加
- 同一Price IDのアイテムが既存の場合はquantity増加（重複作成不可のため）
- 50% OFFクーポン自動適用（テスト: `group_50off` / 本番: `7Vig6pTt`）
- メンバー削除時: quantity > 1 なら減算、1なら削除

### API

| メソッド | パス | 説明 |
|---------|------|------|
| POST | `/api/members/invite` | メンバー追加 |
| GET | `/api/members/list` | メンバー一覧 |
| DELETE | `/api/members/remove` | メンバー削除 |
| PUT | `/api/members/change-plan` | メンバーのプラン変更 |

### UI

- `/members` — メンバー管理ページ（追加フォーム・一覧・削除・月額合計）
- ダッシュボードに「メンバー管理 (N)」ボタン
